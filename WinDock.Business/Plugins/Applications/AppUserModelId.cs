...
namespace WinDock.Plugins.Applications
{
    public class AppUserModelId
    {
        ...
        public string DestinationList
        {
            get
            {
                if (Custom)
                {
                    ...
                    var file = Path.Combine(path, Id.ToLower()) + ".customDestinations-ms";
                    return File.Exists(file) ? file : null;
                }
                else
                {
                    var path = Environment.ExpandEnvironmentVariables(AutomaticDestinationsJumplistFolder);
                    var file = Path.Combine(path, Id.ToLower()) + ".automaticDestinations-ms";
                    return File.Exists(file) ? file : null;
                }
            }
        }

        public static IEnumerable<AppUserModelId> Find(IntPtr windowHandle)
        {
            var explicitId = Api.GetWindowAppId(windowHandle);
            var processId = (int)WindowsManagedApi.User32.Helpers.GetWindowThreadProcessId(windowHandle);
        ...
        public static IEnumerable<AppUserModelId> Find(string executable)
        {
            ...
            //path = TrySubstituteEnvironmentVariable(path);
            var paths = TryReplaceWithKnownFolder(path);
            var appIds = CalculateAppIds(true, Path.GetFileName(executable), paths);
            return appIds.Select(a => new AppUserModelId(executable, a, false)).ToList();
        }
                ...
                foreach (var knownFolder in KnownFolders.All)
                {
                    ...
                    if (splitKnownFolderPath.Length == depth)
                    {
                        if (!String.IsNullOrWhiteSpace(knownFolder.Path) && path.ToLower().StartsWith(knownFolder.Path.ToLower() + Path.DirectorySeparatorChar))
                        {
                            ...
                            if (knownFolder.Path == KnownFolders.System.Path)
                            {
                                return new List<string>
                                    {
                                        "{" + KnownFolders.System.FolderId + "}" + remaining,
                                        "{" + KnownFolders.SystemX86.FolderId + "}" + remaining,
                                    };
                            }

                            if (knownFolder.Path == KnownFolders.ProgramFilesCommon.Path)
                            {
                                return new List<string>()
                                    {
                                        "{" + KnownFolders.ProgramFilesCommon.FolderId + "}" + remaining,
                                        "{" + KnownFolders.ProgramFilesCommonX64.FolderId + "}" + remaining,
                                        "{" + KnownFolders.ProgramFilesCommonX86.FolderId + "}" + remaining
                                    };
                            }

                            if (knownFolder.Path == KnownFolders.ProgramFilesX86.Path)
                            {
                                return new List<string>()
                                    {
                                        "{" + KnownFolders.ProgramFiles.FolderId + "}" + remaining,
                                        "{" + KnownFolders.ProgramFilesX64.FolderId + "}" + remaining,
                                        "{" + KnownFolders.ProgramFilesX86.FolderId + "}" + remaining
                                    };
                            }

                            return new List<string> { "{" + knownFolder.FolderId + "}" + remaining };
                        }
                    }
                }
                depth--;
        ...
        private static IEnumerable<string> CalculateAppIds(bool automatic, string executable, IEnumerable<string> directories)
        {
            ...
            if (automatic)
            {
                var combined = directories.Select(d => d + Path.DirectorySeparatorChar + executable).ToList();
                var formattedNames = combined.Select(CreateFormattedName).ToList();
        ...
        private static class Api
        {
            ...
            /// </remarks>
            [global::System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Portability", "CA1900:ValueTypeFieldsShouldBePortable", MessageId = "_ptr2")]
...
#pragma warning disable 649
                ...
                // architectures.
                [global::System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Reliability", "CA2006:UseSafeHandleToEncapsulateNativeResources")]
                [FieldOffset(12)]
                IntPtr _ptr2;
                [global::System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Reliability", "CA2006:UseSafeHandleToEncapsulateNativeResources")]
                [FieldOffset(8)]
